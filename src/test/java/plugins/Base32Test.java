...
class Base32Test {
    ...
    void givenText_whenEncoding_thenCorrect(){
        ...
        assertDoesNotThrow(() -> {
            byte[] data = coder.convert(text.getBytes());
            assertArrayEquals(
                    expected.getBytes(),
                    data
            );
    ...
    @Test
    void givenEncodedByteArray_whenDecoding_thenCorrect(){
        String text = "ывфываыывцфм";
        assertDoesNotThrow(() -> {
            byte[] encoded = coder.convert(text.getBytes());
            String decoded = new String(coder.revert(encoded));
